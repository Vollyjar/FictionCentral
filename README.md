...
**FictionCentral** is a standalone desktop application built for reading, organizing, and archiving web fiction and fanfiction across 10 major platforms. 
...
- ** High-Performance Downloader**: Multi-threaded background downloads with SQLite Write-Ahead Logging (WAL) for non-blocking concurrent reads and writes.
...
- ** Polite & Ethical Rate Limiting**: Built-in per-domain request throttling (`0.35s`–`0.5s`) and rotating User-Agent pooling to avoid putting unnecessary load on community servers.
...
- **Server Courtesy**: Built-in rate-limiting is implemented so that requests do not overwhelm community hosting infrastructure. 
